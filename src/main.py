	...
	for fila in range(0, NUMERO_FILAS):
		...
		for columna in range(0, NUMERO_COLUMNAS):
			...
			if ESTADO_JUEGO[fila][columna] == 0:
				# Casilla vacia.
				pygame.draw.polygon(ventana, (255, 255, 255, 255), poligono, 1);
...
def controlador_coordenadas(posicion_mouse: list) -> list:
	...
	# !CAMBIE DE LADO ESTAS PARA QUE FUNCIONE EL JUEGO, CON MIS COORDENADAS FUNCIONABA AL REVES
	return list([coordenada_Y, coordenada_X])
...
def controlador_juego() -> None:
    ...
    #Variables para el manejo de turnos(temporal)
	turno_blanca = False
	turno_negra = True
